...
- **Dashboard** (Admin only, first item in the nav and the page Admin lands on after login) — a live, auto-updating view of today's sales: order count, net sales, cash/GCash split, and the top 5 best-selling products for the day. Also includes a product sales lookup with a 1 Week / 1 Month / custom date-range filter to see how many units of a specific product sold in that window.
...
- **Inventory count** (Inventory → Inventory Count) — every ingredient listed at once, pre-filled with current stock, for fast beginning/ending-of-day counts (Admin and Stockman). Admin also has a third option, **Actual Count**, for a physical stock-take that can be run any time rather than tied to day open/close. Submitting any count type reconciles stock to what was counted and logs the variance as a normal adjustment.
- **Spoilage log** (Inventory → Spoilage) — log damaged/expired stock with a reason and an optional photo; deducts stock immediately and keeps a searchable history.
...
- **Roles** — Admin (dashboard, reports, users, settings, products, inventory oversight, receipts/refunds — no POS/register access), Cashier (POS + own shifts + expenses + refunds), Stockman (inventory, purchase orders, suppliers + expenses). Enforced via protected routes.
