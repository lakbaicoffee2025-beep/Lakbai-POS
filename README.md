...
- **Expenses** — cash-envelope reports: record cash/ATM received, itemized spending with receipt photos, and the remaining-cash return status, with a one-tap carryover of yesterday's unreturned balance. Each line item is also mirrored into the plain expense log so it reduces the expected cash-in-drawer total for the shift it's logged against and still shows up in Reports.
