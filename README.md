...
- **Dashboard** (Admin only, first item in the nav and the page Admin lands on after login) — a live, auto-updating view of sales: order count, net sales, cash/GCash split, and the top 5 best-selling products, for a selectable Today / 1 Week / 1 Month / custom date-range window. Also includes a separate product sales lookup with its own 1 Week / 1 Month / custom date-range filter to see how many units of a specific product sold in that window. **Shift Sales Breakdown** (shown right above Top 5 Products): always defaults to whichever shift is currently open (or the most recent one if none is), so there's always a live shift in view without having to pick one — quantity sold per product with a By Category filter showing totals per category, and an hourly sales bar chart across the shift's duration (each hour of the shift gets a bar, 0 if nothing sold that hour). A dropdown lets Admin switch to any other shift, open or closed, from any cashier, to review it instead.
- **POS** — category-filtered product grid, size variants, modifiers (milk options, add-ons, sweetness, etc.), per-line and order-level discounts, Cash / GCash / split payment, an optional customer name, and a printable receipt. A grid/list view toggle lets a cashier switch the product picker between the visual card grid and a denser list layout — the choice is remembered per device. **Dark mode**: a 🌙 toggle (visible even before opening a shift) switches the whole cashier experience — POS (product grid, cart, checkout, receipt, every POS dialog), Shift (cash count, paid outs, shift reports), and Receipts — to a dark theme for low-light counters. The toggle is shared and stays in sync across all three screens and browser tabs; the rest of the app (nav, admin pages) is unaffected, and the preference is remembered per device. Selecting a size/modifier is never required, and a sale can go through even for an item flagged out of stock. **Open Tickets**: hold a cart before payment under a name/notes (e.g. a table or customer) and resume it later — handy for a customer who steps away or a phoned-in order. Held tickets are scoped to the shift that created them (they won't resurface — or be chargeable — once that shift is closed and a new one is opened) and, like the in-progress cart draft, live only on the device that held them, never synced across registers, so they can't be silently overwritten by another device's stale copy. The cart currently being built is also auto-saved per shift in the background, so an accidental page reload or crash mid-sale doesn't lose it — it's restored automatically the next time that shift is open on that device. Discounts can be turned off store-wide from Settings (Admin only) — when off, cashiers see no discount picker at checkout at all; the discount list itself (Products → Discounts) is always admin-only to create, edit, or enable/disable.
...
- **Movement Log** (Inventory → Movement Log, Admin only) — every stock change (sales, purchase receipts, adjustments, waste, count reconciliations), with a Day / Week / Custom view (Custom keeps the old From/To free-range picker) and a CSV export of whatever's in view. Day and Week modes each show a Sales vs. Ending Count summary above the raw list — the same expected-stock-minus-sales logic as Reports → Reconciliation — so a discrepancy is visible right away without leaving the log; Week mode rolls that up per day (tap a day to drill into it) and still links back to the same reason-badge flagging (Discrepancy/Final) on the individual movement entries below.
...
- **Reports** — daily sales & top products, full shift history, expense breakdown by category/date range, inventory valuation & low-stock alerts. **Reconciliation**: for any day, each ingredient's expected stock (its beginning-of-day count minus what sold that day) is compared against what the stockman actually counted at close, surfacing discrepancies sales alone don't explain (restocked/wasted quantities are shown alongside for context, not netted in, so a gap that lines up with logged waste is easy to rule out). A Week view rolls this up into a 7-day history — how many ingredients were off and the estimated value at stake each day — so a recurring problem stands out; tap a day to drill into its per-ingredient detail. CSV export for a day's reconciliation.
